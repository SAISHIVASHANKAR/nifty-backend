...
def compute_all_indicators(df, symbol, cursor):
    ...
    if not all(col in df.columns for col in required_cols):
        ...
        print(f"âš ï¸ Skipping {symbol}: missing columns {missing}")
        return
    ...
    try:
        ...
        df['GANN'] = (df['high'] + df['low']) / 2  # placeholder for Gann fan
        ...
        print(f"âŒ Indicator computation failed for {symbol}: {e}")
